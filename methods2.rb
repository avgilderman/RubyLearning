...
def sklonenie(one, two, three, four)
  # проверим входные данные на правильность
  if (one == nil || !one.is_a?(Numeric))
    one = 0 # если первый параметр пустой или не число, то продолжаем как будто он нулевой
  end

  ostatok = one % 10 # склонение определяется последней цифрой в числе
